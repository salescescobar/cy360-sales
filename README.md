...
- **CourtReserve live API** (spec section 10 — optional; CSV upload works without it): set `COURTRESERVE_API_USER`, `COURTRESERVE_API_PASS`, `COURTRESERVE_ORG_ID`, and flip `sources.courtreserve.mode` to `api` in `config.yaml`. `npm run backfill:courtreserve -- --from=YYYY-MM-DD --to=YYYY-MM-DD` pages the range month by month into `sales_transactions`/`court_reservations`/`payment_type_totals` (payment-basis detail) AND `revenue_recognized` (the report's actual source, spec v5 section 3 — service-date basis) — idempotent, replaces per location/range, never duplicates. Member PII (`MemberFullName`/`FamilyName`/`MemberFirstName`/`MemberLastName`) is dropped before anything is persisted, and any occurrence of a member's name inside the free-text `Description` field is redacted too — checked against every member name seen in that fetch, not just the row's own, since a group/HOA-type fee is often billed to one member while the free text names someone else (`scrubMemberNames` in `packages/skills/courtreserve-ingest/index.ts`).
...
- **The v5 report** is business-line-first, not source-first: `/dashboard/<location>` resolves every recognized/GoTab row through `business_line_map` (seeded by `supabase/migrations/0005_recognized_revenue.sql`) into Food & Beverage · Pickleball · Memberships · Events · Lessons & Classes · Swag · Arcade · Sponsorships → Gross Revenues · Discounts · Total, each with this-month-to-date / prior-month / same-month-last-year columns. `/admin/business-lines` lists anything Unmapped and lets an admin assign it (writes `business_line_map`, never drops it silently). `/admin/reconciliation` shows the known recognized-vs-payment-basis discrepancy (spec section 4) per FeeCategory/TransactionType so the CEO — not this code — decides the recognition rule in `config.yaml -> report.recognition`.
